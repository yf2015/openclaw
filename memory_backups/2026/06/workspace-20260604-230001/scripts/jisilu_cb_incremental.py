...
def acquire_lock():
    """获取运行锁，防止并发重入。锁文件10分钟内有效，超时视为过期。"""
    LOCK_DIR = Path('/root/.openclaw/workspace/data')
    LOCK_DIR.mkdir(exist_ok=True)
    LOCK_FILE = LOCK_DIR / "jisilu_fetch.lock"
    if LOCK_FILE.exists():
        try:
            mtime = LOCK_FILE.stat().st_mtime
            if time.time() - mtime < 600:  # 10分钟内不重复（小于cron间隔，防竞态）
                return False
        except: pass
...
def main():
    ...
    try:
        ...
        try:
            ...
            # 全部完成通知（先通知，再保存"完成"状态，避免竞态导致重复抓取）
            if all(p["slices"]):
                import subprocess
                ...
                try:
                    r = subprocess.run(['curl', '-s', '-X', 'POST', WEBHOOK,
                                       '-H', 'Content-Type: application/json',
                                       '-d', payload],
                                      capture_output=True, text=True, timeout=10)
                    print(f"  钉钉通知响应: {r.stdout.strip()}")
