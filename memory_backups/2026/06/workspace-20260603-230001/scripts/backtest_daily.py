...
HTML_OUT      = "/home/www/toolbox-api/static/stock/huice/bt_report.html"
...
# ── K线补采 akshare 无认证方案 ───────────────────────────────
def ensure_prices_ak(conn, date, days=10):
    """akshare 方案：无需Cookie，数据源标记为 'akshare'"""
    cur = conn.cursor()
    print(f"  [{date}] 从akshare补采K线...")
    fetched = 0
    for name, code in NAME_TO_CODE.items():
        sym = f"sh{code}" if code.startswith(("11","13")) else f"sz{code}"
        try:
            ...
            for _, row in df.tail(days).iterrows():
                ...
                cur.execute("""
                    ...
                    DO UPDATE SET open=excluded.open,high=excluded.high,low=excluded.low,
                                  close=excluded.close,volume=excluded.volume,amount=excluded.amount
                """, (code, trade_date, row['open'], row['high'], row['low'], row['close'],
                      int(row['volume']), 0))
            conn.commit()
            fetched += 1
        except Exception as e:
            print(f"    akshare失败 {name}({sym}): {e}")
        time.sleep(0.3)
    print(f"  akshare补采完成: {fetched} 只转债")
...
GROUPS = [
    (1, [1.0]),           # G1 前1只满仓
    (2, [0.0, 1.0]),      # G2 前2只——第2只满仓（用户新增）
    (2, [0.5, 0.5]),      # G3 前2只均仓
    (2, [2/3, 1/3]),      # G4 前2只——第1只2/3仓 第2只1/3仓
    (2, [1/3, 2/3]),      # G5 前2只——第1只1/3仓 第2只2/3仓
]
...
def build_html(results):
    ...
    return (
        ...
        + section_html(2, "G2 前 2 只（第 2 只满仓）", s2)
        + section_html(3, "G3 前 2 只（均仓 50/50）", s3)
        + section_html(4, "G4 前 2 只（第1只 2/3 仓 第2只 1/3 仓）", s4)
        + section_html(5, "G5 前 2 只（第1只 1/3 仓 第2只 2/3 仓）", s5)
        ...
        f'<tr><td>G2 第2只满仓</td><td class="text-right">{s2["capital"]:,.2f}</td>'
        ...
        f'<tr><td>G3 前2只均仓</td><td class="text-right">{s3["capital"]:,.2f}</td>'
        ...
        f'<tr><td>G4 第1只2/3仓 第2只1/3仓</td><td class="text-right">{s4["capital"]:,.2f}</td>'
        f'<td class="text-right">{s4["total_ret"]:+.2f}%</td>'
        f'<td class="text-right">{s4["ann"]:+.2f}%</td><td class="text-right">-{s4["max_dd"]:.2f}%</td>'
        f'<td class="text-right">{s4["sharpe"]:.4f}</td></tr>\n'
        f'<tr><td>G5 第1只1/3仓 第2只2/3仓</td><td class="text-right">{s5["capital"]:,.2f}</td>'
        f'<td class="text-right">{s5["total_ret"]:+.2f}%</td>'
        f'<td class="text-right">{s5["ann"]:+.2f}%</td><td class="text-right">-{s5["max_dd"]:.2f}%</td>'
        f'<td class="text-right">{s5["sharpe"]:.4f}</td></tr>\n'
        '</tbody>\n</table>\n'
...
def main():
    ...
    cur  = conn.cursor()
    # 优先取 akshare，无则降级到 xueqiu
    cur.execute("SELECT sec_code,trade_date,open,close FROM klines_xq WHERE source='akshare'")
    price_map = {(r[0], r[1]): (float(r[2]), float(r[3])) for r in cur.fetchall()}
    print(f"  K线记录(akshare): {len(price_map)} 条")

    if daily_bonds:
        latest = max(daily_bonds.keys())
        ensure_prices_ak(conn, latest)
        cur.execute("SELECT sec_code,trade_date,open,close FROM klines_xq WHERE source='akshare'")
        price_map = {(r[0], r[1]): (float(r[2]), float(r[3])) for r in cur.fetchall()}

        # akshare 仍未覆盖全量，降级到雪球补漏
        if len(price_map) < 50:
            print(f"  akshare 数据不足({len(price_map)}条)，降级到雪球...")
    ...
    # 文件已通过docker卷挂载到nginx容器，不再需要docker cp
    ...
    payload = {
        ...
        "markdown": {
            ...
            "text": (
                ...
                f"| G2 第2只满仓 | {s2['capital']:,.0f} | {s2['total_ret']:+.2f}% | {s2['sharpe']:.4f} |\n"
                f"| G3 前2只均仓 | {s3['capital']:,.0f} | {s3['total_ret']:+.2f}% | {s3['sharpe']:.4f} |\n"
                f"| G4 第1只2/3 第2只1/3 | {s4['capital']:,.0f} | {s4['total_ret']:+.2f}% | {s4['sharpe']:.4f} |\n"
                f"| G5 第1只1/3 第2只2/3 | {s5['capital']:,.0f} | {s5['total_ret']:+.2f}% | {s5['sharpe']:.4f} |\n\n"
                f"👉 查看: http://121.41.72.70:9002/stock/huice/bt_report.html"
            )
        }
    }
