...
def build_report_v2(results, today_dt):
    ...
    msg += '| 维度 | ' + n1 + ' | ' + n2 + ' | ' + n3 + ' |\n'
    msg += '|---|---|---|---|' + '\n'
    msg += '| **V2评分** | **' + fmt(r1['_v2_score']) + '** | **' + fmt(r2['_v2_score']) + '** | **' + fmt(r3['_v2_score']) + '** |\n'
    msg += '| 风险等级 | ' + cnt1 + ' | ' + cnt2 + ' | ' + cnt3 + ' |\n'
    msg += '| 溢价得分 | ' + fmt(r1['_prem_score']) + ' | ' + fmt(r2['_prem_score']) + ' | ' + fmt(r3['_prem_score']) + ' |\n'
    msg += '| 计数得分 | ' + fmt(r1['_count_score']) + ' | ' + fmt(r2['_count_score']) + ' | ' + fmt(r3['_count_score']) + ' |\n'
    msg += '| 超触发得分 | ' + fmt(r1['_trigger_score']) + ' | ' + fmt(r2['_trigger_score']) + ' | ' + fmt(r3['_trigger_score']) + ' |\n'
    msg += '| 现价 | ' + fmt(r1.get('price')) + '元 | ' + fmt(r2.get('price')) + '元 | ' + fmt(r3.get('price')) + '元 |\n'
    msg += '| 溢价率 | ' + fmt(r1.get('premium')) + '% | ' + fmt(r2.get('premium')) + '% | ' + fmt(r3.get('premium')) + '% |\n'
    msg += '| 正股超触发 | ' + a1 + '% | ' + a2 + '% | ' + a3 + '% |\n'
    msg += '| 剩余规模 | ' + scale_tag(r1) + ' | ' + scale_tag(r2) + ' | ' + scale_tag(r3) + ' |\n'
...
def main():
    ...
    print('[' + now_str + '] B型不强赎博弈策略 V1+V2 开始...')
    ...
    for b in bonds:
        ...
        if cnt and not is_no_redeem and above_trigger is not None and above_trigger >= 0:
            ...
            results.append(r)
            print('  命中: ' + detail['name'] + '(' + bid + ') ' + str(cnt) + ' 溢价:' + str(round(premium, 2)) + '% → V2评分:' + fmt(r['_v2_score']))
    ...
    top3_v1 = v1_results[:3]
    top3_v2 = v2_results[:3]
    print('  最终命中: ' + str(len(results)) + ' 只')
    print('  V1 top3: ' + ', '.join(r['name'] for r in top3_v1))
    print('  V2 top3: ' + ', '.join(r['name'] for r in top3_v2))

    # ── 推送 ────────────────────────────────────────
    report_v1 = build_report_v1(top3_v1, today_dt)
    report_v2 = build_report_v2(top3_v2, today_dt)

    # 对比摘要
    v1_names = [r['name'] for r in top3_v1]
    v2_names = [r['name'] for r in top3_v2]
    diff_note = ''
