...
L, C, R = 12, 16, 16  # 标签 | 华兴列 | 超达列

def top():  return '  ' + '┌' + '─'*L + '┬' + '─'*C + '┬' + '─'*R + '┐'
def mid():  return '  ' + '├' + '─'*L + '┼' + '─'*C + '┼' + '─'*R + '┤'
def bot():  return '  ' + '└' + '─'*L + '┴' + '─'*C + '┴' + '─'*R + '┘'
def sep():  return '  ' + '├' + '─'*L + '┼' + '─'*C + '┼' + '─'*R + '┤'
def hrow(a,b,c): return '  ' + a.ljust(L)+'│'+b.ljust(C)+'│'+c.ljust(R)+'│\n'
def srow(a,b,c): return '  ' + '│'.join([a.ljust(L),b.ljust(C),c.ljust(R)])+'│\n'
...
def get_detail(bid, session):
    ...
    def fv(key):
        raw = val_map.get(key, '')
        try: return float(re.search(r'[\d.]+', raw).group())
        except: return None
...
def build_report(results, today_dt):
    ...
    def gamble_tag(r):
        ...
        total = 0
        if c[0] <= 9: total += 20
        if '-' not in str(r.get('stock_roe') or '-'): total += 20
        if (r.get('premium') or 0) < 2: total += 15
        if (r.get('shareholder_ratio') or 0) >= 70: total += 15
        if (r.get('remain_years') or 0) >= 2: total += 10
        if total >= 65: return '⭐⭐⭐⭐⭐'
        elif total >= 45: return '⭐⭐⭐⭐'
        elif total >= 25: return '⭐⭐⭐'
        else: return '⭐⭐'
    def gamble_risk(r):
        c = r['count']
        tags = []
        if c[0] >= 13: tags.append('接近触发')
        if (r.get('premium') or 0) < 0: tags.append('负溢价')
        if '-' in str(r.get('stock_roe') or '-'): tags.append('ROE亏')
    ...
    msg += '| 剩余年限 | ' + yrs_tag(r1) + ' | ' + yrs_tag(r2) + ' |\n'
    msg += '| 已转股比例 | ' + conv_tag(r1) + ' | ' + conv_tag(r2) + ' |\n'
    msg += '| 股东配售率 | ' + sh_tag(r1) + ' | ' + sh_tag(r2) + ' |\n'
    ...
    def st_risk_tag(r):
        ...
        if nm.startswith('*') or nm.startswith('ST') or 'ST' in nm: parts.append('🔴正股ST')
        roe_v = str(r.get('stock_roe') or '')
        try:
            if '-' not in roe_v and float(roe_v) < -10: parts.append('🔴ROE极差')
        except: pass
        rating_v = str(r.get('rating') or '')
        if rating_v in ('A-', 'BBB', 'BB', 'B', 'CCC', 'CC', 'C'): parts.append('🟡评级差')
        return '/'.join(parts) if parts else '✅无异常'
    msg += '| ST/退市风险 | ' + st_risk_tag(r1) + ' | ' + st_risk_tag(r2) + ' |\n'
    msg += '| 博弈价值 | ' + gamble_tag(r1) + ' | ' + gamble_tag(r2) + ' |\n'
...
def main():
    ...
    top2 = results[:2]
    print('  最终: ' + str(len(results)) + ' 只，取前' + str(len(top2)))
    report = build_report(top2, today_dt)
    print('')
    print('=' * 60)
    print(report)
