...
import requests, re, datetime as dt, holidays

# ── 交易日判断 ─────────────────────────────────────────────
def is_trading_day(d=None):
    """"判断是否为A股交易日（排除周末+中国节假日）"""
    if d is None:
        d = dt.date.today()
    cn = holidays.China(years=d.year)
    return d.weekday() < 5 and d not in cn

def is_trading_time(now=None):
    """判断当前是否在交易时间段内"""
    if now is None:
        now = dt.datetime.now().time()
    return (dt.time(9, 15) <= now <= dt.time(11, 30)) or (dt.time(13, 0) <= now <= dt.time(15, 0))
