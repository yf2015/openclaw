    ...
    try:
        ...
        for a in annos:
            ...
            if is_redeem_decision(a["title"]) and bond_id not in redeem_map:
                redeem_map[bond_id] = {**name_map.get(bond_id, {"bond_nm": "-"}), "announce_date": a["date"], "title": a["title"], "url": a["url"]}
                redeem_map[bond_id]["bond_nm"] = name_map.get(bond_id, "-")
            elif is_no_redeem_decision(a["title"]) and bond_id not in no_redeem_map:
                no_redeem_map[bond_id] = {**name_map.get(bond_id, {"bond_nm": "-"}), "announce_date": a["date"], "title": a["title"], "url": a["url"]}
                no_redeem_map[bond_id]["bond_nm"] = name_map.get(bond_id, "-")
    except Exception:
        continue
...
for label, bond_map in [("强赎", redeem_map), ("不强赎", no_redeem_map)]:
    ...
    for idx, item in enumerate(items):
        ...
        bond_id = item.get("bond_id", list(bond_map.keys())[idx])
        klines = get_kline(ss, bond_id)
        if klines:
            feats = extract_features(klines)
            if feats:
                item["bond_id"] = bond_id
                item["features"] = feats
                item["klines"] = klines
...
print(f"\n已保存: /root/.openclaw/workspace/jisilu_redeem_analysis.json")